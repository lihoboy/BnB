    ...
    while(len(bb_tree)!=1):
        ...
        if Current_node.Count_choosed_Jobs>=Jobs_toDo-1:
            ...
            for i in range(Current_node.Count_choosed_Jobs+1, Jobs_toDo):
                ...
                '''Domi_r_K'''
                Domi_r_k_flag=0
                if r_K_1<=Current_node.Cmax_soFar<r_K and Current_node.Cmax_soFar+Jobs_list[Current_node.Jseq[i]-1].processing_length>=r_K and Jobs_list[Current_node.Jseq[i]-1].release_date<r_K:
                    for larger_than_r_k in range(Current_node.Count_choosed_Jobs + 1, Jobs_toDo):
                        if i !=larger_than_r_k and Jobs_list[Current_node.Jseq[larger_than_r_k]-1].processing_length<=Jobs_list[Current_node.Jseq[i]-1].processing_length and Current_node.Cmax_soFar+Jobs_list[Current_node.Jseq[larger_than_r_k]-1].processing_length>=r_K:
